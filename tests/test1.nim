...
test "external_module : mutateMeBaby[Array]":
...
test "external_module : testMeBaby[Tensor]":
  var testMeBaby = nimjl_get_function(jl_main_module, "testMeBaby")
  check not isNil(testMeBaby)

  var dims: seq[int] = @[4, 6, 8]
  let dims_ptr = cast[ptr UncheckedArray[int]](dims[0].addr)
  var array_dims: nimjl_value  = nimjl_apply_array_type_int64(1)
  var xDims= nimjl_ptr_to_array_1d(array_dims, dims_ptr, dims.len.csize_t, 1)
  ...
  var xTensor = nimjl_ptr_to_array(array_type, orig.get_data_ptr(), xDims, 1)
  var ret : nimjl_value = nimjl_call1(testMeBaby, xTensor)
...
test "external_module : mutateMeBaby[Tensor]":
  var mutateMeBaby = nimjl_get_function(jl_main_module, "mutateMeBaby!")
  check not isNil(mutateMeBaby)
  var testMeBaby = nimjl_get_function(jl_main_module, "testMeBaby")
  check not isNil(testMeBaby)
  ...
  let orig_ptr = orig.get_data_ptr()
  var array_type : nimjl_value  = nimjl_apply_array_type_float64(3)
  var xTensor= nimjl_ptr_to_array(array_type, orig_ptr, xDims, 1)
  var ret : nimjl_value = nimjl_call1(mutateMeBaby, xTensor)
  check not isNil(ret)
